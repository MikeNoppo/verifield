...
- **This package lives at `apps/backend/` inside a Bun workspace.** The frontend sits beside it at `apps/frontend/`, the generated API contract at `packages/contract/`, and `docs/` at the repo root is the shared business specification. Run every command in this file from `apps/backend/` — or from the repo root via `bun run --filter @verifield/backend <script>`.
