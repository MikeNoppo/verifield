...
import { listInspectorTasks } from "@/lib/api/orders"
import { inspectorActions } from "@/lib/domain/status"
import { CURRENT_INSPECTOR_ID } from "@/lib/mock/seed"
import { tanggalJam } from "@/lib/format"

export default function TugasHariIni() {
  const tasks = listInspectorTasks(CURRENT_INSPECTOR_ID)

  return (
    <>
      <h1 className="mb-1 text-lg font-semibold tracking-tight">Tugas Saya</h1>
      <p className="mb-4 text-xs text-muted-foreground">
        {tasks.length} penugasan berjalan
      </p>

      <SyncQueueBanner count={2} />
